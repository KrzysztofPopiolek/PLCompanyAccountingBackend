...
//    public List<Summary> getAllMonthlySummary(@PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Date date) {
//        List<Summary> monthlySummaries = monthlySummaryRepository.findAll();
//        List<Summary> foundMonthlySummaries = new ArrayList<>();
//        for (int i = 0; i < monthlySummaries.size(); i++) {
//            if (monthlySummaries.get(i).getDate().compareTo(date) == 0) {
